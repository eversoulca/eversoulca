...
class GeminiService {
  ...
  static GenerativeModel _getModel() {
    if (_apiKey == null || _apiKey!.isEmpty) {
      throw Exception('Gemini API key not initialized. Please set GEMINI_API_KEY in .env');
    }
  ...
  static Future<String> generateInterviewQuestion(
    ...
    try {
      ...
      final context = history.isNotEmpty
          ? '当前对话记录如下：\n${history.join('\n')}'
          : '目前还没有对话。';

      final prompt = '''你是一位专业的人生传记作家。当前的主题是："$currentTopic"。
...
4. 每次只提一个问题，且不超过20个字。''';
      ...
      print("Gemini interview generation failed: $err");
  ...
  static Future<String> generateNextPrompt(String userMemory) async {
    try {
      final model = _getModel();
      final prompt = '''基于用户分享的这段经历："$userMemory"，请作为一个专注且具有同理心的记录者，提一个深情且能引发更多往事细节的后续问题（不超过15个字）。目标是让对方感到被尊重和理解，并愿意分享更多人生智慧。请称呼对方为"您"，不要使用任何长辈/晚辈的称呼。''';

      final response = await model.generateContent([Content.text(prompt)]);
      return response.text?.trim() ?? "那时候还有什么让您难忘的事吗？";
    } catch (err) {
      print("Gemini prompt generation failed: $err");
  ...
  static Future<String> generateEmpathicResponse(String userVenting) async {
    try {
      final model = _getModel();
      final prompt = '''用户正在倾诉一段情绪或心事："$userVenting"。请作为一个深沉、睿智、平等且从不评判的"聆听者"，给出一段温暖、包容且极具同理心的回应（不超过25个字）。语气要像灵魂伴侣或挚友，让用户感到安全。''';

      final response = await model.generateContent([Content.text(prompt)]);
      return response.text?.trim() ?? "我都听到了，这里永远是你安静的避风港。";
    } catch (err) {
      print("Gemini venting response failed: $err");
